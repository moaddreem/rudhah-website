    ...
    <div className="min-h-screen" dir="ltr">
      ...
      <section className="relative bg-gradient-to-b from-primary via-primary to-primary/95 text-bg py-16 md:py-20 overflow-hidden">
        ...
        <div className="absolute inset-0 opacity-10">
          <div className="absolute top-10 right-10 w-32 h-32 border border-accent/50 rounded-full"></div>
          <div className="absolute bottom-20 left-10 w-48 h-48 border border-accent/30 rounded-full"></div>
          <div className="absolute top-1/2 right-1/4 w-24 h-24 border border-bg/20 rounded-full"></div>
        </div>
        ...
        <div className="max-w-[1280px] mx-auto px-4 sm:px-6 lg:px-8">
          <FadeIn>
            <div className="text-center mb-10">
              <span className="text-lg sm:text-sm md:text-base font-ui tracking-wider text-accent">From Our Menu</span>
      ...
      <section className="py-10 md:py-16">
        <div className="max-w-[1280px] mx-auto px-4 sm:px-6 lg:px-8">
          <FadeIn>
            <div className="text-center mb-10">
              <span className="text-lg sm:text-sm md:text-base font-ui tracking-wider text-accent">What They Say</span>
              <h2 className="text-[2rem] sm:text-2xl md:text-3xl font-mishafi font-bold text-primary mt-2">
                Testimonials
              </h2>
              <div className="w-16 h-1 bg-accent mx-auto mt-4 rounded-full"></div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              {siteData.testimonials.map((testimonial, index) => (
                <div key={index} className="bg-cream rounded-xl p-6 relative">
                  <svg className="w-8 h-8 text-accent/30 absolute top-4 left-4" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M14.017 21v-7.391c0-5.704 3.731-9.57 8.983-10.609l.995 2.151c-2.432.917-3.995 3.638-3.995 5.849h4v10h-9.983zm-14.017 0v-7.391c0-5.704 3.748-9.57 9-10.609l.996 2.151c-2.433.917-3.996 3.638-3.996 5.849h3.983v10h-9.983z"/>
                  </svg>
                  <p className="text-primary mb-4 leading-relaxed text-lg sm:text-base">"{testimonial.text_en}"</p>
                  <p className="text-accent font-bold text-base sm:text-sm">— {testimonial.author_en}</p>
                </div>
              ))}
            </div>
          </FadeIn>
              ...
              <div className="bg-cream rounded-lg p-4 mb-6">
                <div className="flex justify-center gap-8 text-base">
                  <div>
                    ...
                    <p className="text-primary font-bold text-sm">{siteData.hours.weekdays.open_en} - {siteData.hours.weekdays.close_en}</p>
                  </div>
                  <div className="w-px bg-muted/30"></div>
                  <div>
                    <p className="text-muted mb-1">{siteData.hours.friday.day_en}</p>
                    <p className="text-primary font-bold text-sm">{siteData.hours.friday.open_en} - {siteData.hours.friday.close_en}</p>
                  </div>
                </div>
              </div>
